...
const ModuleCard = ({ title, description, features, icon, colorClass, buttonText, link }) => {
  const colorStyles = {
    mental: {
      ...
      text: "text-green-700",
      dotBg: "bg-linear-to-br from-green-500 to-green-600",
    },
    career: {
      bg: "bg-blue-50",
      ...
      text: "text-blue-700",
      dotBg: "bg-linear-to-br from-blue-500 to-blue-600",
    },
    resources: {
      bg: "bg-orange-50",
      ...
      dotBg: "bg-linear-to-br from-orange-500 to-orange-600",
    },
  }
  ...
  return (
    <div className={`${styles.bg} border ${styles.border} shadow-md hover:shadow-lg group rounded-2xl p-5 transition-all duration-300 hover:-translate-y-2 cursor-pointer`}>
      {/* Icon */}
      <div className={`w-12 h-12 rounded-xl ${styles.iconBg} flex items-center justify-center mb-4 shadow-md group-hover:scale-110 transition-transform duration-300`}>
      ...
      <ul className="space-y-1.5 mb-5">
        {features.map((feature, index) => (
          <li key={index} className="flex items-start gap-2 text-sm text-gray-600">
            <div className={`w-1.5 h-1.5 rounded-full ${styles.dotBg} mt-1.5 shrink-0`} />
            <span>{feature}</span>
          </li>
        ))}
      </ul>
      ...
      {/* Button */}
      <Link to={link}>
        <button className={`w-full px-4 py-2.5 rounded-lg font-semibold text-sm transition-all hover:scale-[1.02] ${colorClass === 'mental'
          ? 'bg-linear-to-r from-green-500 to-green-600 text-white hover:shadow-lg'
          : colorClass === 'career'
            ? 'bg-linear-to-r from-blue-500 to-blue-600 text-white hover:shadow-lg'
            : 'bg-linear-to-r from-orange-500 to-orange-600 text-white hover:shadow-lg'
          }`}>
          {buttonText}
        </button>
      </Link>
